...
func (v volumeAttachmentInfos) Less(i, j int) bool {
	if v[i].MachineId < v[j].MachineId {
		return true
	} else if v[j].MachineId < v[i].MachineId {
		return false
	}
	if v[i].UnitId < v[j].UnitId {
		return true
	} else if v[j].UnitId < v[i].UnitId {
		return false
	}
	if v[i].Storage < v[j].Storage {
		return true
	} else if v[j].Storage < v[i].Storage {
		return false
	}

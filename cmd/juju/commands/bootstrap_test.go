...
func (s *BootstrapSuite) run(c *gc.C, test bootstrapTest) testing.Restorer {
	...
	controllerDetails, err := store.ControllerByName("peckham-controller")
	c.Assert(err, jc.ErrorIsNil)
	models, err := store.AllModels("peckham-controller")
	c.Assert(err, jc.ErrorIsNil)
	c.Assert(models, gc.HasLen, 1)
	_, ok := models["admin"]
	c.Assert(ok, jc.IsTrue)

	cfg := opFinalizeBootstrap.InstanceConfig.Config
	c.Assert(cfg.Name(), gc.Equals, "admin")
	caCert, hasCert := cfg.CACert()
	c.Check(hasCert, jc.IsTrue)
	uuid, hasUUID := cfg.UUID()
	c.Check(hasUUID, jc.IsTrue)
	_, hasKey := cfg.CAPrivateKey()
	c.Check(hasKey, jc.IsTrue)
	c.Assert(controllerDetails.APIEndpoints, jc.DeepEquals, []string{addrConnectedTo})
	c.Assert(controllerDetails.Servers, jc.DeepEquals, []string{"localhost"})
	c.Assert(controllerDetails.CACert, gc.Equals, caCert)
	c.Assert(controllerDetails.ControllerUUID, gc.Equals, uuid)
...
func (s *BootstrapSuite) TestBootstrapSetsCurrentController(c *gc.C) {
	s.patchVersionAndSeries(c, "raring")
	...
	currentEnv, err := modelcmd.ReadCurrentController()
	c.Assert(err, jc.ErrorIsNil)
...
func (s *BootstrapSuite) TestBootstrapControllerExists(c *gc.C) {
	const controllerName = "devenv"
	s.patchVersionAndSeries(c, "raring")

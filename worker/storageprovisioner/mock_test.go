...
func (v *mockVolumeAccessor) DestroyVolumeParams(volumes []names.VolumeTag) ([]params.DestroyVolumeParamsResult, error) {
	var result []params.DestroyVolumeParamsResult
	for _, tag := range volumes {
		v, ok := v.provisionedVolumes[tag.String()]
		if !ok {
			result = append(result, params.DestroyVolumeParamsResult{
				Error: &params.Error{Code: params.CodeNotProvisioned},
			})
			continue
		}
		volumeParams := params.DestroyVolumeParams{
			Provider: "dummy",
			VolumeId: v.Info.VolumeId,
			Release:  tag.Id() == releasingVolumeId,
		}
		result = append(result, params.DestroyVolumeParamsResult{Result: volumeParams})
	}
	return result, nil
}
...
func (v *mockFilesystemAccessor) DestroyFilesystemParams(filesystems []names.FilesystemTag) ([]params.DestroyFilesystemParamsResult, error) {
	results := make([]params.DestroyFilesystemParamsResult, len(filesystems))
	for i, tag := range filesystems {
		f, ok := v.provisionedFilesystems[tag.String()]
		if !ok {
			results = append(results, params.DestroyFilesystemParamsResult{
				Error: &params.Error{Code: params.CodeNotProvisioned},
			})
			continue
		}
		filesystemParams := params.DestroyFilesystemParams{
			Provider:     "dummy",
			FilesystemId: f.Info.FilesystemId,
			Release:      tag.Id() == releasingFilesystemId,
		}
		results[i] = params.DestroyFilesystemParamsResult{Result: filesystemParams}
	}
	return results, nil
}

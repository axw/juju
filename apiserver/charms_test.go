...
func (s *charmsSuite) TestPOSTRequiresAuth(c *gc.C) {
	resp := sendHTTPRequest(c, httpRequestParams{method: "POST", url: s.charmsURI("")})
...
func (s *charmsSuite) TestGETRequiresAuth(c *gc.C) {
	resp := sendHTTPRequest(c, httpRequestParams{method: "GET", url: s.charmsURI("")})
	body := assertResponse(c, resp, http.StatusUnauthorized, "text/plain; charset=utf-8")
	c.Assert(string(body), gc.Equals, "authentication failed: no credentials provided\n")
}

func (s *charmsSuite) TestRequiresPOSTorGET(c *gc.C) {
	resp := s.sendHTTPRequest(c, httpRequestParams{method: "PUT", url: s.charmsURI("")})
	body := assertResponse(c, resp, http.StatusMethodNotAllowed, "text/plain; charset=utf-8")
...
func (s *charmsSuite) TestPOSTRequiresUserAuth(c *gc.C) {
	...
	resp = s.sendHTTPRequest(c, httpRequestParams{method: "POST", url: s.charmsURI("")})
...
func (s *charmsSuite) TestGetAllowsTopLevelPath(c *gc.C) {
	...
	url.Path = "/charms"
	resp := s.sendHTTPRequest(c, httpRequestParams{method: "GET", url: url.String()})
	s.assertGetFileResponse(c, resp, "1", "text/plain; charset=utf-8")
}
...
func (s *charmsSuite) TestNoTempFilesLeftBehind(c *gc.C) {
	...
	uri := s.charmsURI("?url=local:quantal/dummy-1&file=*")
	resp := s.sendHTTPRequest(c, httpRequestParams{method: "GET", url: uri})
	assertResponse(c, resp, http.StatusOK, "application/zip")

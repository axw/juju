...
func (c Config) Validate() error {
	...
	if c.BootstrapTimeout <= 0 {
		return errors.NotValidf("%s %s", BootstrapTimeoutKey, c.BootstrapTimeout)
	}
	if c.BootstrapRetryDelay <= 0 {
		return errors.NotValidf("%s %s", BootstrapRetryDelayKey, c.BootstrapRetryDelay)
	}
	if c.BootstrapAddressesDelay <= 0 {
		return errors.NotValidf("%s %s", BootstrapAddressesDelayKey, c.BootstrapAddressesDelay)
	}
	return nil
}
...
func NewConfig(controllerUUID string, attrs map[string]interface{}) (Config, error) {
	...
	if caCert, ok := attrs[CACertKey].(string); ok {
		...
		var err error
		config.CACert, err = readFileAttr(attrs, CACertKey, CACertKey+".pem")
	...
	if caPrivateKey, ok := attrs[CAPrivateKeyKey].(string); ok {
		...
		var err error
		config.CAPrivateKey, err = readFileAttr(attrs, CAPrivateKeyKey, CAPrivateKeyKey+".pem")
		if err != nil && os.IsNotExist(errors.Cause(err)) {
	...
	if config.CACert == "" && config.CAPrivateKey == "" {
		...
		if config.CACert != "" {
			return Config{}, errors.NewNotValid(err, fmt.Sprintf(
				"%s is set, but %s is not", CACertKey, CAPrivateKeyKey,
			))
		}
		if config.CAPrivateKey != "" {
			return Config{}, errors.NewNotValid(err, fmt.Sprintf(
				"%s is set, but %s is not", CAPrivateKeyKey, CACertKey,
			))
		}
	}
...
// path.
func readFileAttr(attrs map[string]interface{}, key, defaultPath string) (string, error) {
	if !osenv.IsJujuXDGDataHomeSet() {
		return "", errors.Errorf("$JUJU_DATA is not set, cannot read %q", key)
	}
	...
	if err != nil {
		return "", errors.Annotatef(err, "%q not set, and could not read from %q", path)
	}
	if len(data) == 0 {
		return "", errors.Errorf("file %q is empty", path)
	}

...
func (s *bootstrapSuite) TestBootstrapGUISuccessLocal(c *gc.C) {
	...
	env := newEnviron("foo", useDefaultKeys, nil)
	ctx := coretesting.Context(c)
	err := bootstrap.Bootstrap(modelcmd.BootstrapContext(ctx), env, bootstrap.BootstrapParams{ControllerConfig: coretesting.FakeControllerConfig()})
	c.Assert(err, jc.ErrorIsNil)
...
func (s *bootstrapSuite) TestPublicKeyEnvVar(c *gc.C) {
	...
	env := newEnviron("foo", useDefaultKeys, nil)
	err := bootstrap.Bootstrap(envtesting.BootstrapContext(c), env, bootstrap.BootstrapParams{ControllerConfig: coretesting.FakeControllerConfig()})
	c.Assert(err, jc.ErrorIsNil)

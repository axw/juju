...
func Login(session *mgo.Session, user, password string) error {
	...
	if err := admin.Login(user, password); err != nil {
		return MaybeUnauthorized(errors.Annotatef(err, "cannot log in to admin database as %q", user))
	}
	return nil
}
...
func MaybeUnauthorized(err error) error {
	...
	return errors.Wrap(err, errors.Unauthorizedf("mongo access unauthorized"))
}

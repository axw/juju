...
func (env *sessionEnviron) MaintainInstance(args environs.StartInstanceParams) error {
	return env.env.MaintainInstance(args)
}
...
func (env *sessionEnviron) newRawInstance(
	...
	vmName, err := env.env.namespace.Hostname(args.InstanceConfig.MachineId)
	if err != nil {
	...
	externalNetwork := env.env.ecfg.externalNetwork()
	if externalNetwork != "" {
		apiPort := 0

...
func (ctxt *httpContext) stateForRequestAuthenticated(r *http.Request) (
	...
	if !ok {
		return nil, nil, nil, common.ErrPerm
	}
	...
	if authInfo.Controller {
		entityState = ctxt.srv.statePool.SystemState()
	}
	entity, err := entityState.FindEntity(authInfo.Tag)
	if err != nil {
		releaser()
		return nil, nil, nil, errors.Trace(err)
	}
...
func (controllerAuthorizer) Authorize(authInfo httpcontext.AuthInfo) error {
	...
	return errors.Errorf("%s is not a controller", names.ReadableString(authInfo.Tag))
}

...
// and names.DiskTag.
func VolumeToState(v params.Volume) (names.DiskTag, state.VolumeInfo, error) {
	if v.VolumeTag == "" {
		return names.DiskTag{}, state.VolumeInfo{}, errors.New("Tag is empty")
	}
	volumeTag, err := names.ParseDiskTag(v.VolumeTag)
	if err != nil {
		return names.DiskTag{}, state.VolumeInfo{}, errors.Trace(err)
	}

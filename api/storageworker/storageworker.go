...
const storageWorkerFacade = "StorageWorker"

// State provides access to a storageworker's view of the state.
type State struct {
	facade base.FacadeCaller
	scope  names.Tag
}

// NewState creates a new client-side StorageWorker facade.
func NewState(caller base.APICaller, scope names.Tag) *State {
	return &State{
		base.NewFacadeCaller(caller, storageWorkerFacade),
		scope,
	}
}
...
func (st *State) Volumes(tags []names.DiskTag) ([]params.VolumeResult, error) {
	args := params.Entities{
...
func (st *State) VolumeParams(tags []names.DiskTag) ([]params.VolumeParamsResult, error) {
	args := params.Entities{
		Entities: make([]params.Entity, len(tags)),
	}
	for i, tag := range tags {

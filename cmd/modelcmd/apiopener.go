...
type APIOpener interface {
	Open(store jujuclient.ClientStore, connectionName string) (api.Connection, error)
}

type OpenFunc func(jujuclient.ClientStore, string) (api.Connection, error)

func (f OpenFunc) Open(store jujuclient.ClientStore, name string) (api.Connection, error) {
	return f(store, name)
}
...
func (t *timeoutOpener) Open(store jujuclient.ClientStore, name string) (api.Connection, error) {
	...
	go func() {
		api, dialErr := t.opener.Open(store, name)
		if dialErr != nil {
			errc <- dialErr
			return
		}

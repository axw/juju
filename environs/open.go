...
func New(config *config.Config) (Environ, error) {
	p, err := Provider(config.Cloud().Type)
	if err != nil {
...
func prepare(
	...
	if args.Credential.AuthType() != "" {
		cloudCredentialAttrs := args.Credential.Attributes()
		cloudCredentialAttrs["auth-type"] = string(args.Credential.AuthType())
		attrs[config.CredentialsKey] = cloudCredentialAttrs
	}
	...
	cfg, err = p.BootstrapConfig(BootstrapConfigParams{cfg, args.Credential})
	if err != nil {
		return nil, details, errors.Trace(err)
	}
